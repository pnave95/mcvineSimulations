...
def convolution_1D(params, final_length, debug=0):
	...
	# now, combine bins to reduce the total number of bins to the same number of bins as L2
	...
	reduced_Z_bins = np.zeros(L2)
	reduced_Z_prob = np.zeros(L2)
	for i in range(L2):
		total_prob = 0.0
		...
		if total_prob == 0.0:
			com = Z_bins_list_sorted[i*L1 + L1 - 1] - Z_bins_list_sorted[i*L1]
		else:
			com /= total_prob
...
def multiply_distributions(X, Y, final_length=-1, debug=0):
	...
	Z = [reduced_Z_bins, reduced_Z_prob]
	#Z = [np.array(Z_bins_list_sorted), np.array(Z_prob_list_sorted)]
...
def retrieve_t12_distribution(pathToOutDirectory, final_length=-1, debug=0):
	...
	bins_mt1 = []
	prob_mt1 = []
	for i in range(len(prob_minus_t1)):
		if prob_minus_t1[i] > 0.000:
			prob_mt1.append(prob_minus_t1[i])
			bins_mt1.append(bins_minus_t1[i])
	bins_t2a = []
	prob_t2a = []
	for i in range(len(prob_t2)):
		if prob_t2[i] > 0.000:
			prob_t2a.append(prob_t2[i])
			bins_t2a.append(bins_t2[i])
	prob_minus_t1 = np.array(prob_mt1)
	bins_minus_t1 = np.array(bins_mt1)
	prob_t2 = np.array(prob_t2a)
	bins_t2 = np.array(bins_t2a)
	if debug == 1:
		print "len(prob_minus_t1) = " + str(len(prob_minus_t1))
		print "len(prob_t2) = " + str(len(prob_t2))

	# if final_length= -1, then use the first beam monitor data length
	if final_length == -1:
		final_length = len(prob_t2)
...
def get_vi_distribution(d12, t12, final_length=-1, debug=0):
	...
	inv_t12_bins = np.zeros(len(t12_bins))#1.0 / t12_bins
	index = 0
	for bin in t12_bins:
		if bin != 0.0:
			inv_t12_bins[index] = 1.0 / bin
		else:
			print "zero in t12_bins"
		index += 1
