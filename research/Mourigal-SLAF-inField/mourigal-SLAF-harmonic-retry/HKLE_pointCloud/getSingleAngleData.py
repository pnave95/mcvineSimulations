...
def rawInfo(ifile, E_i):
	...
	outfile = stripped_input + "-Data.csv"
	out = open(outfile, 'w+')  # overwrite file if exists
	out.write("H.K,L,E,weight,Qx,Qy,Qz,polar,azimuthal\n")
	...
	for bank in range(115):
		...
		#for e in range(10):	
			...
			info = str(H) + "," + str(K) + "," + str(L) + "," + str(EmeV) + "," + str(w) + "," + str(Qx) + "," + str(Qy) + "," + str(Qz) + "," + str(polar) + "," + str(azimuth) + "\n"
			out.write(info)
